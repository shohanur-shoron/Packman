...
static void  __stdcall iA0(HWND,UINT, UINT_PTR, DWORD){if(!iAnimPause[0])iAnimFunction[0]();}
static void  __stdcall iA1(HWND,UINT, UINT_PTR, DWORD){if(!iAnimPause[1])iAnimFunction[1]();}
static void  __stdcall iA2(HWND,UINT, UINT_PTR, DWORD){if(!iAnimPause[2])iAnimFunction[2]();}
static void  __stdcall iA3(HWND,UINT, UINT_PTR, DWORD){if(!iAnimPause[3])iAnimFunction[3]();}
static void  __stdcall iA4(HWND,UINT, UINT_PTR, DWORD){if(!iAnimPause[4])iAnimFunction[4]();}
static void  __stdcall iA5(HWND,UINT, UINT_PTR, DWORD){if(!iAnimPause[5])iAnimFunction[5]();}
static void  __stdcall iA6(HWND,UINT, UINT_PTR, DWORD){if(!iAnimPause[6])iAnimFunction[6]();}
static void  __stdcall iA7(HWND,UINT, UINT_PTR, DWORD){if(!iAnimPause[7])iAnimFunction[7]();}
static void  __stdcall iA8(HWND,UINT, UINT_PTR, DWORD){if(!iAnimPause[8])iAnimFunction[8]();}
static void  __stdcall iA9(HWND,UINT, UINT_PTR, DWORD){if(!iAnimPause[9])iAnimFunction[9]();}
...
void iShowBMP2(int x, int y, char filename[], int ignoreColor)
{
    ...
    GLubyte *pixel_data = new GLubyte[width * height * 4];
    int pixel_index = 0;

    for (int i = 0; i < width * height; i++)
    {
        int r = data[i * channels];
        int g = (channels > 1) ? data[i * channels + 1] : r;
        int b = (channels > 2) ? data[i * channels + 2] : r;
        int a = (channels > 3) ? data[i * channels + 3] : 255;

        if (r == 0 && g == 0 && b == 0 && ignoreColor == 0)
        {
            a = 0;
        }

        pixel_data[pixel_index++] = r;
        pixel_data[pixel_index++] = g;
        pixel_data[pixel_index++] = b;
        pixel_data[pixel_index++] = a;
    }
